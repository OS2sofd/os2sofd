...
public class PersonController {
    ...
    public String createPerson(Model model, @Valid @ModelAttribute("createPersonDTO") CreatePersonDTO createPersonDTO, BindingResult bindingResult) {
        ...
        auditLogger.log(person.getUuid(), EntityType.PERSON, EventType.PERSON_CREATE, PersonService.getName(person), null);
    ...
    public String view(Model model, @PathVariable("uuid") String uuid, @RequestParam(required = false, value = "backRef") String backRef) {
        ...
        model.addAttribute("constraintOUs", SecurityUtil.getOrgUnitUuidsFromConstraint());
        ...
        if (!managedOrgUnits.isEmpty()) {
            for (OrgUnit ou : managedOrgUnits) {
                personDTO.getSubstituteAssignments().addAll(ou.getSubstitutes().stream().map(a -> new SubstituteAssignmentDTO(a)).toList());

                findInheritedSubstitutes(ou.getParent(), personDTO, ou.getName());
            }
        }
    ...
    private void findInheritedSubstitutes(OrgUnit parent, PersonDTO personDTO, String managedOrgUnitName) {
        if (parent == null) {
            return;
        }
        ...
        for (SubstituteOrgUnitAssignment substituteOrgUnitAssignment : parent.getSubstitutes()) {
            if (substituteOrgUnitAssignment.getContext().isInheritOrgUnitAssignments()) {
                personDTO.getSubstituteAssignments().add(new SubstituteAssignmentDTO(substituteOrgUnitAssignment, managedOrgUnitName, true, parent.getName()));
            }
        }

        findInheritedSubstitutes(parent.getParent(), personDTO, managedOrgUnitName);
    }
    ...
    public String createNewAffiliation(Model model, @ModelAttribute("personUUID") String personUUID, @Valid @ModelAttribute("affiliationDTO") AffiliationDTO affiliationDTO, BindingResult bindingResult) {
        ...
        String message = "Der er oprettet et nyt tilhørsforhold med stillingsbetegnelsen " + aff.getPositionName() + " i enheden " + aff.getCalculatedOrgUnit().getName();
    ...
    public ResponseEntity<byte[]> downloadPersonHistory(@PathVariable("uuid") String uuid, @PathVariable("revId") Integer revId) {
        ...
        if (revision != null) {
            try {
                ...
                json = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(new HistoryPerson(revision.getEntity()));
    ...
    private PersonDTO toDTO(Person person) {
        PersonDTO personDTO = PersonDTO.builder()
                ...
                .leaveExpireAccounts((person.getLeave() != null) ? person.getLeave().isExpireAccounts() : false)
