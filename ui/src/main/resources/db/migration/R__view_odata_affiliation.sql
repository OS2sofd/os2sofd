CREATE OR REPLACE ALGORITHM=TEMPTABLE VIEW view_odata_affiliation_test AS
  SELECT affiliations.id                         AS id,
         ...
         COALESCE(workplace.orgunit_uuid,
                  affiliations.alt_orgunit_uuid,
                  affiliations.orgunit_uuid)     AS orgunit_uuid,
         ...
         CASE
           WHEN TRIM(IFNULL(affiliations.position_display_name,'')) <> ''
             THEN affiliations.position_display_name
           WHEN prof.id IS NOT NULL
             ...
             THEN prof.name
           ELSE affiliations.position_name
